...
describe("scanSourceContent", () => {
  ...
  it("returns empty string for missing pages directory", async () => {
    const content = await scanSourceContent("/nonexistent/path");
    expect(content).toBe("");
  });

  it("returns empty string for empty pages directory", async () => {
    const emptyDir = await Deno.makeTempDir({
      prefix: "source-scanner-empty-",
    });
    await Deno.mkdir(join(emptyDir, "pages"), { recursive: true });
